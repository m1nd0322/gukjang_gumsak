...
구형 `WooriRenewal` HTML 화면이나 Selenium DOM에 의존하지 않습니다. JSON 응답 구조와 Snapshot·ShareAnalysis의 실제 종목코드를 검증하므로, HTTP 200 오류 문서와 우선주→보통주 페이지 연결도 결과에서 걸러냅니다.
...
- `nps_state.json`이 보유 기준선과 활성 신호를 로컬에 유지하며, GitHub Actions에서는 같은 파일을 실행 간 캐시로 복원·저장합니다.
...
1. 이전 실행의 `nps_state.json` 캐시 복원
2. Python 3.11 및 의존성 설치
3. 회귀 테스트 실행
4. FnGuide 세 지표 수집과 종목 스코어링
5. 2점 이상 종목의 6개월 복합 전략 백테스트
6. yfinance 가격과 KOSPI 벤치마크 수집
7. 텔레그램 요약/CSV 전송
8. 성공한 국민연금 상태를 다음 실행용 캐시에 저장
9. CSV를 GitHub Actions Artifact로 30일 보관
