...
### 2026-09-13 데이터 확보 및 검증 현황

**현재 판정: 생존편향 완전 검증 미완료(`blocked`).** 상장일은 수집 목록의 1,434종목 모두 확보했고, 기간 내 상장폐지 256종목의 KRX 원시가격 242,731행 수집과 vendor 대조를 마쳤습니다. 원시가격 확보는 수정주가 인증이나 모든 거래일의 완전성 검증을 뜻하지 않습니다.

| 항목 | 확보 또는 실행 결과 | 남은 한계 |
| --- | --- | --- |
| 가격 차이 | 256종목 중 211종목의 OHLC 차이 확인 | 수정주가 인증 완료 종목은 0개 |
| 분배금 대조 | 차이 211종목 중 149종목에 SEIBRO 이익분배 내역 존재 | 62종목은 확보된 이익분배 내역 없음; 기업행동이 없었다는 뜻은 아님 |
| 가격비율 변화 | 0.2% 초과 변화 682건 중 674건이 지급기준일 ±7일에 위치 | 지급기준일은 분배락일이 아니며 근접성은 조정 산식의 증명이 아님 |
| SEIBRO | 이익분배 10,271건, 청산분배 252건 수집 | 조회 건수 대비 수집 완료일 뿐 전체 과거 사건의 완전성은 미검증 |
| 동일 조건 백테스트 시도 | 전체 목록으로 `--cached --universe-mode both` 실행, 종료 코드 1 | 조정가격 캐시 부족으로 수익률 계산 전에 차단 |
| 현재 상장 가격 | 기존 전략 8종목의 KRX 연도별 원시가격 수집 착수 | 마지막 확인은 2종목 7,836행; 8종목 또는 전체 현재 상장 종목 완료로 해석하지 않음 |

청산금액 미확보 종목은 `097740`, `105450`, `110550`, `124090`이며,
`427110`은 운용사 공지와 SEIBRO 금액의 불일치가 남아 있습니다.
마지막 종가를 실제 청산금액으로 대체하거나, 미검증 원시가격을 조정가격으로
표시해 실행 차단을 우회하지 않습니다.

웹에서 원문 6건을 추가 보관했습니다. KRX 배포 2008년 상장 안내, 삼성의
2016년 말 달러선물 ETF 자료, 한화의 2021-07-30 기준 ETF 목록은 과거 상품
특성의 근거 후보입니다. 이 자료들은 프로젝트의 과거 선정 규칙이나 당시
전체 모집단을 증명하지 않습니다. 공시 재게시본과 언론 보도는 공식 원문 및
실제 지급 확인 자료와 구분합니다.

기존 평가기의 `both` 옵션만으로 동일 조건 비교가 보장되지는 않습니다.
각 비교군이 평가 달력과 위험예산을 따로 선택하므로, 최종 비교에서는 공통
평가기간과 고정 파라미터를 명시적으로 적용해야 합니다. 날짜별 전략 매핑,
전체 기업행동, 실제 청산 내역과 양쪽 비교군의 검증된 조정가격도 필요합니다.

상세 내용은 [가격 진단 및 백테스트 실행 결과](reports/krx_price_and_paired_backtest_status_20260912.md)와
[웹 데이터 확보 현황 및 출처](reports/krx_web_data_receipt_20260913.md)에 있습니다.
문서에 언급된 상세 JSON, 다운로드 원문 및 진행 중 수집 상태는 로컬 산출물이며
이번 문서 커밋에는 포함하지 않습니다.
